...
class MixinHandler(object):
    ...
    def initialize(self, loop=None):
        ...
        self.cors_origins = self.settings.get('cors_origins', [])
        self.check_request()
    ...
    def check_origin(self, origin):
        ...
        if self.cors_origins and (origin in self.cors_origins or '*' in self.cors_origins):
            return True
    ...
    def set_default_headers(self):
        ...
        cors_origins = getattr(self, 'cors_origins', set())
...
class ApiSSHHandler(MixinHandler, tornado.web.RequestHandler):
    """API handler for SSH connections at /api/ssh"""

    executor = ThreadPoolExecutor(max_workers=cpu_count()*5)

    def initialize(self, loop, policy, host_keys_settings):
        super(ApiSSHHandler, self).initialize(loop)
        self.policy = policy
    ...
    def set_default_headers(self):
        ...
        origin = self.request.headers.get('Origin', '')
        cors_origins = getattr(self, 'cors_origins', set())
        if origin and (origin in cors_origins or '*' in cors_origins or 'http://localhost:5173' in cors_origins):
            self.set_header('Access-Control-Allow-Origin', origin)
            self.set_header('Access-Control-Allow-Methods', 'GET, POST, OPTIONS')
            self.set_header('Access-Control-Allow-Headers', 'Content-Type')

    def check_origin(self, origin):
        return True  # Allow all origins for API
    ...
    def post(self):
        ...
        if workers and len(workers) >= options.maxconn:
            self.set_status(403)
            self.write(dict(id=None, status='Too many live connections', encoding=None))
            return
        ...
        try:
            ...
            self.set_status(400)
            self.write(dict(id=None, status=str(exc), encoding=None))
            return
    ...
    def lookup_hostname(self, hostname, port):
        ...
        if self.ssh_client._system_host_keys.lookup(key) is None:
            if self.ssh_client._host_keys.lookup(key) is None:
                raise InvalidValueError('Connection to {}:{} is not allowed.'.format(hostname, port))
